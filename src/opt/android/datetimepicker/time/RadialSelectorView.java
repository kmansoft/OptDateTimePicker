...
public class RadialSelectorView extends View {
    ...
    // kman
    private boolean mIsIndentStart;
    ...
    public void initialize(Context context, boolean is24HourMode, boolean hasInnerCircle,
        ...
        int blue = res.getColor(R.color.opt_dtpicker_blue);
        ...
        if (is24HourMode) {
            mCircleRadiusMultiplier = Float.parseFloat(
                    res.getString(R.string.opt_dtpicker_circle_radius_multiplier_24HourMode));
        } else {
            mCircleRadiusMultiplier = Float.parseFloat(
                    res.getString(R.string.opt_dtpicker_circle_radius_multiplier));
            mAmPmCircleRadiusMultiplier =
                    Float.parseFloat(res.getString(R.string.opt_dtpicker_ampm_circle_radius_multiplier));
        }
        ...
        if (hasInnerCircle) {
            mInnerNumbersRadiusMultiplier =
                    Float.parseFloat(res.getString(R.string.opt_dtpicker_numbers_radius_multiplier_inner));
            mOuterNumbersRadiusMultiplier =
                    Float.parseFloat(res.getString(R.string.opt_dtpicker_numbers_radius_multiplier_outer));
        } else {
            mNumbersRadiusMultiplier =
                    Float.parseFloat(res.getString(R.string.opt_dtpicker_numbers_radius_multiplier_normal));
        }
        mSelectionRadiusMultiplier =
                Float.parseFloat(res.getString(R.string.opt_dtpicker_selection_radius_multiplier));
    ...
    /* package */ void setTheme(Context context, boolean themeDark) {
        ...
        int color;
        if (themeDark) {
            // kman color = res.getColor(R.color.red);
            color = res.getColor(R.color.opt_dtpicker_blue_dark);
            mSelectionAlpha = SELECTED_ALPHA_THEME_DARK;
        } else {
            color = res.getColor(R.color.opt_dtpicker_blue);
            mSelectionAlpha = SELECTED_ALPHA;
        }
        mPaint.setColor(color);
    }
    
    public void setIndentStart(boolean indent) {
    	if (mIsIndentStart != indent){
    		mIsIndentStart = indent;
    		invalidate();
    	}
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        int startY = mYCenter;
        if (mIsIndentStart) {
        	int startLen = (int)(mLineLength * 0.25f);
        	startX = mXCenter + (int) (startLen * Math.sin(mSelectionRadians));
        	startY = mYCenter - (int) (startLen * Math.cos(mSelectionRadians));
        }

        canvas.drawLine(startX, startY, pointX, pointY, mPaint);
    }
