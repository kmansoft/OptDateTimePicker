...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	public RadialPickerLayout(Context context, AttributeSet attrs) {
		...
		mGrayBox.setBackgroundColor(getResources().getColor(R.color.opt_dtpicker_transparent_black));
	...
	public void initialize(Context context, HapticFeedbackController hapticFeedbackController, int initialHoursOfDay,
		...
		final boolean isDarkTheme = Utils.isThemeDark(colorPrimary.getDefaultColor());
		a.recycle();
		// kman end
	...
	public void setIsCenterHourView(boolean centerHourView) {
		mIsCenterHourView = centerHourView;
	}

	private boolean mIsCenterHourView;
	...
	public void setTime(int hours, int minutes) {
		...
		mAmPmCirclesView.setAmOrPm(hours < 12 ? AM : PM);
	}
	 ...
	 * selectable area to each of the 12 visible values, such that the ratio of space apportioned to
	 * a visible value : space apportioned to a non-visible value will be 14 : 4. E.g. the output of
	 * 30 degrees should have a higher range of input associated with it than the output of 24
	 * degrees, because 30 degrees corresponds to a visible number on the clock circle (5 on the
	 * minutes, 1 or 13 on the hours).
	 */
	...
	/**
	 * Returns mapping of any input degrees (0 to 360) to one of 60 selectable output degrees, where
	 * the degrees corresponding to visible numbers (i.e. those divisible by 30) will be weighted
	 * heavier than the degrees corresponding to non-visible numbers. See
	 * {@link #preparePrefer30sMap()} documentation for the rationale and generation of the mapping.
	 */
	 ...
	 *            The output may be forced to either the higher or lower step, or may be allowed to
	 *            snap to whichever is closer. Use 1 to force strictly higher, -1 to force strictly
	 *            lower, and 0 to snap to the closer one.
	 ...
	 * @param isInnerCircle
	 *            Whether the selection should be in the inner circle; will be ignored if there is
	 *            no inner circle.
	 * @param forceToVisibleValue
	 *            Even if the currently-showing circle allows for fine-grained selection (i.e.
	 *            minutes), force the selection to one of the visibly-showing values.
	 * @param forceDrawDot
	 *            The dot in the circle will generally only be shown when the selection is on
	 ...
	 *            The y coordinate.
	 * @param forceLegal
	 *            Force the selection to be legal, regardless of how far the coordinates are from
	 *            the actual numbers.
	 * @param isInnerCircle
	 *            If the selection may be in the inner circle, pass in a size-1 boolean array here,
	 *            inside which the value will be true if the selection is in the inner circle, and
	 *            false if in the outer circle.
	...
	/**
	 * kman: added animation "chaining"
	 */
	public void setCurrentItemShowing(int index, boolean animate) {
		setCurrentItemShowing(index, animate, null);
	}
	 ...
	 * 
	 * @param animate
	 *            True to animate the transition, false to show with no animation.
	 */
	public void setCurrentItemShowing(int index, boolean animate, ObjectAnimator addAnimator) {
		...
		if (animate && (index != lastIndex)) {
			ObjectAnimator[] anims = new ObjectAnimator[4 + (addAnimator != null ? 1 : 0)];
		...
		if (mIsCenterHourView) {
			mCircleView.setDrawDot(index == HOUR_INDEX);
			mMinuteRadialSelectorView.setIndentStart(index == MINUTE_INDEX);
		}
	}
